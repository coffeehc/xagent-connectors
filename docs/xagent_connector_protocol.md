...
模型调用工具时的 arguments 必须包含 `connector_channel_id`：
...
xAgent adapter 必须先从模型参数中提取、校验 `connector_channel_id`，再把它映射到 packet envelope 顶层。Connector data plane 收到的 wire packet：
...
- 模型参数中的 `connector_channel_id` 是标准保留路由参数，不属于具体工具业务 arguments。
- xAgent adapter 发送 `tool.invoke` 前必须从 `arguments` 中移除 `connector_channel_id`，并写入 envelope 顶层 `connector_channel_id`。
- Connector 业务工具不应把 `connector_channel_id` 当作普通业务参数消费，也不允许解释为目标系统账号 ID。
- wire packet 的 `payload.arguments` 只能包含具体工具业务参数，不包含 `connector_channel_id` 这类标准保留参数。
- `context` 是 xAgent 运行时上下文，Connector 只能作为关联信息使用，不能当鉴权材料。
- 缺少、无归属、Connector 不匹配或不可用 channel 必须报错，禁止自动补齐或切换 channel。
...
| `connector_channel_id_required` | 模型工具参数或用户级 packet 缺少 `connector_channel_id` |
...
- `connector_channel_id` 不属于具体工具业务 arguments；xAgent adapter 会把它映射为 `tool.invoke` envelope 顶层 `connector_channel_id`。
- 系统 API key、目标系统 token、transfer token 不进入 `input_schema`。
- 发送文件类工具应只消费 `media_ref`，不消费文件字节、base64 或目标系统 URL。

### 10.1 LLM channel discovery 与显式路由

LLM 调用任何 Connector 工具前必须明确目标 `connector_channel_id`：

1. 同源回复：如果当前输入来自 Connector 入站事件，优先使用事件投影中的 `source_connector_channel_id`。
2. 跨 Connector、主动发送或多账号场景：先调用 xAgent 提供的 `connector_channels_list` 工具，查询当前用户可用的精确 channel。
3. 模型调用 Connector 工具时，在工具参数中填写 `connector_channel_id`。
4. xAgent adapter 校验该 channel 属于当前用户、匹配当前 Connector Card、状态可用且工具可用。
5. xAgent adapter 从模型参数中移除 `connector_channel_id`，并写入 `tool.invoke` packet 顶层 `connector_channel_id`。
6. Connector Server 按 packet 顶层 `connector_channel_id` 路由和校验目标系统登录态。

禁止以下兜底语义：

- 未传 `connector_channel_id` 时选择第一个 channel。
- 当前用户只有一个 channel 时自动补齐。
- 从当前连接、历史调用、Connector 名称、provider、ConnectorCardID 或账号名称猜测 channel。
- channel 与 Connector 不匹配时自动切换到其他 channel。

错误语义：

- 缺少 `connector_channel_id`：xAgent adapter 必须拒绝模型工具调用，错误码建议 `connector_channel_id_required`。
- channel 不属于当前用户：拒绝调用，错误码建议 `connector_channel_not_owned`。
- channel 不属于当前 Connector Card 或工具来源：拒绝调用，错误码建议 `connector_channel_connector_mismatch`。
- channel 存在但未连接、未认证、工具不可用或权限未授予：拒绝调用，错误码建议 `connector_channel_unavailable` 或更具体的 `connection_not_authenticated`。

不要定义 `connector_card_id` 模型参数。Connector Card 可以通过 `user_id + connector_channel_id` 的绑定关系反查并校验。
...
4. 向模型投影 `source_connector_channel_id = envelope.connector_channel_id`，用于同源回复时原样填入 Connector 工具参数。
5. 将 Connector Card 或 Connection Descriptor 的来源标签补到可见消息前。
6. 将 `media` / `media_refs` 转为 `download_url` ResourceRef。
7. 提交为 `SessionEvent`，由 Agent 主链消费。
