...
func (impl *serviceImpl) registerRoutes(app *fiber.App) {
	...
	log.Debug("Feishu connector HTTP 路由已注册",
...
func (impl *serviceImpl) handleConnectorSkill() fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if err != nil {
			log.Debug("读取 Feishu connector skill 失败",
...
func mediaUploadErrorStatus(err error) int {
	...
	message := err.Error()
	switch {
	case strings.Contains(message, "required"), strings.Contains(message, "empty"), strings.Contains(message, "not bound"), strings.Contains(message, "only image"), strings.Contains(message, "exceeds"):
...
func (impl *serviceImpl) authorizedConnectorRequest(c fiber.Ctx) (bool, error) {
	...
	log.Debug("拒绝 Feishu connector HTTP 请求",
